REQUIREMENTS

ALREADY DONE

1. Add subjects classification of books*
2. Add Course*
3. Instead of ISBN, copyright year*
4. Add text for Acquisition Number*
5. Instead of MiddleName, Middle INitial*
6. Separate button for most borrowed book*
7. Inventory of Books by Section*
- pa.add ako bago na user type.Employee.bale an ea privilege same la han instructor gah..salamat * 
- Dapat an course nkadisable kun an usertype admin ngan teacher*
- Disable first ManageRFID button or messagebox para save muna an book bago i.manage rfid..*
- arrange books alphabetically *
- arrange alphabetical for user's listview *
- confirm password textbox must be clear after saving in User form *
- messagebox if entering letters in acquisition number *
- messagebox for successfully saving acquisitions *
- error messagebox for saving duplicate acquisitions *
- gah alayon nla lwt ak pagbutang messagebox kun an username already exists..*
- reset manage rfid values upon closed (remove recent read tag)
- pwede adto makita an tanan na list han books?..pero myda ghp adto na search na feature. taz apihon nla an number of copies, call number,..taz available *
--if instructor or admin usertype no my payments button, an transactions la ngan book search..total mkikita mn ha ea transaction an status han book if returned or not.. *

---------------------------------------------------------------------------------------------------------------------

TODO LIST

an about ghp han pagreturn han books..kun dri an naghuram an nag.uli..
an dati kasi db an name tapos may button na return..pwd ba adto ig.alphabet?..siguro naman rare la ito na may magpapauli hano?..so ok la kun dri hea nkalist..drinman cgro madamo an lista..butangi nla lwt messagebox..asking kun sure ba an admin na i.click adto na button..hehe..

--Add button in mainwindow "My Transaction" to show all the transactions done by the client..arranged by recent transaction date..include status of books if returned or not yet

- search books by title in books'listview
- in Users, search by lastname in listview
- Acquisition in listview ha managerfid form


- If RFID book id added dapat dri na nablink/highlight
- Add Cancel Button ha Manage RFID form ???????
- Fix Holidays, Holidays list view
- add Acquisition in report

ha UI gah..an most borrowed books..list of most borrowed books only..an ha yna kasi tanan na list han books nakikita..cgro an mga books na nahuhuram 10x ha usa ka semana..api na hea ha list of most borrowed books..hehe

gah ha Client page pag.add man CHange password na feature..thanks

-------------------
* done




gah..gntry ko gnclick an mga retun..ngkawagra an list ha for clearance..mski wa pa bayad..
gah adi man..sunod2 nla gah..hehe..
UIPayment button - add messagebox if there's no records yet..

gah adi gah..sunod2 nla..hehe..
UI project
*payment button -bale gah an ea function para ha mag.uuli libro na dri hra naghuram..para liwat kun mgbayad..bngn kasi nagpadara nla lwt bayad..
--add messagebox if there's no records yet
--arrange the list alphabetical and group the books by students..(kay kun damo an ea gnhuram..under la ha ea name. pero an kada book myda return na button then pag click mpakiana if sure na himuon ito na transaction..if yes..pwd another messagebox pra i.ask kun babaydan na ba o dri..kay kun babaydan na..click yes..kun dri pa..mkadto ea name ha report na for clearance..)

*borrowed books button - ini naman an ea function..baga hea gah report la na list of books na nahuram..bale an sulod hini na report..
book title
author
call no.
date borrowed
expected return date
status (if returned or not yet)
--add messagebox if ther's no records yet..

gah adi gah..sunod2 nla..hehe..
UI project
*payment button -bale gah an ea function para ha mag.uuli libro na dri hra naghuram..para liwat kun mgbayad..bngn kasi nagpadara nla lwt bayad..
--add messagebox if there's no records yet
--arrange the list alphabetical and group the books by students..(kay kun damo an ea gnhuram..under la ha ea name. pero an kada book myda return na button then pag click mpakiana if sure na himuon ito na transaction..if yes..
an ea baga button na event magiging "pay" na..para pwd na bydan o dri..pag dri pa ngani bydan..aadto la ghp ea name ha list..pero kun bydan na..mwawara na ha list..)
bale gah an "for clearance" na report..an tanan na aada ha listahan han payments na form mapa.pay pa mn an event o return..
bale kun an ea event return pa..ok la kun waray pa an amount to pay..manual n an pagcompute since drinpa mn maaram kun sano ig.uuli..
kun an event naman pay na..it means inuli n an book wa pa nbaydan an penalty..so myda na ito amount to pay.)

*borrowed books button - ini naman an ea function..baga hea gah report la na list of books na nahuram..bale an sulod hini na report..
book title
author
call no.
date borrowed
expected return date
status (if returned or not yet)
--add messagebox if theres no records yet..

CLIENT PROJECT
*My Transaction button - list of all transactions done..bale an nakabutang hini na report..
book title
author
call number
acquisition number
date borrowed
expected return date
returned date
penalty (amount to pay)
status (if paid or not kun may baraydan ba..)
--bale gah an pinakrecent na transaction an mauuna ha list..
ito la anay gah unaha..saka na an iba..anxa kasi major eh..smile emoticon
hehe

salamat
-- tapos an screen gah an nkikita la na borrowed tub2 la tulo..an iba ala na..
ito la anay gah unaha..saka na an iba..anxa kasi major eh..smile emoticon
hehe

salamat